...
class TestDepthEstimator(TestCase):
    ...
    def setUp(self) -> None:
        ...
        data_sample.gt_depth_map = PixelData(data=torch.rand(1, 480, 640))
        data_sample.set_metainfo(
            dict(img_shape=(480, 640), ori_shape=(480, 640)))
        self.data = dict(inputs=inputs, data_samples=[data_sample])

    def test_slide_flip_inference(self):

        self.model.test_cfg = ConfigDict(
            dict(mode='slide_flip', crop_size=(480, 480), stride=(160, 160)))
        ...
        self.assertListEqual(list(out[0].pred_depth_map.shape), [480, 640])

    def test__forward(self):
        data = deepcopy(self.data)
        data['inputs'] = data['inputs'][:, :, :480, :480]
        with torch.no_grad():
            out = self.model._forward(**data)
        self.assertListEqual(list(out.shape), [1, 1, 480, 480])
