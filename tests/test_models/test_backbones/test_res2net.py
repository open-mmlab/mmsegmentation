...
from mmseg.models.backbones.res2net import Bottle2neck, Res2Net
...
def test_res2net_backbone():
    ...
    # Test Res2Net with OS=8, scales=4, base_width=26, strides=(1, 1, 1, 1).
    model = Res2Net(depth=50, scales=4, base_width=26, strides=(1, 2, 1, 1))
    for m in model.modules():
        if is_block(m):
            assert m.scales == 4
    model.init_weights()
    ...
    assert feat[0].shape == torch.Size([1, 256, 56, 56])
    assert feat[1].shape == torch.Size([1, 512, 28, 28])
    assert feat[2].shape == torch.Size([1, 1024, 28, 28])
    assert feat[3].shape == torch.Size([1, 2048, 28, 28])
