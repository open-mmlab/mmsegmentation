...
from mmseg.models.decode_heads import PSPCRFHead
from .utils import _conv_has_norm, to_cuda


def test_psp_crf_head():

    with pytest.raises(AssertionError):
        # pool_scales must be list|tuple
        PSPCRFHead(in_channels=32, channels=16, num_classes=19, pool_scales=1)

    # test no norm_cfg
    head = PSPCRFHead(in_channels=32, channels=16, num_classes=19)
