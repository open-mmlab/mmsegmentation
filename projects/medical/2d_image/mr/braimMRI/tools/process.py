...
# images = glob.glob(os.path.join('/mnt/petrelfs/guosizheng/gsz/2d_fenge_todo/braimMRI/images/train', '*.tif'))  # noqa

# masks = glob.glob(os.path.join('/mnt/petrelfs/guosizheng/gsz/2d_fenge_todo/braimMRI/masks/train', '*.tif'))  # noqa

# for i in tqdm(images):
#     img = Image.open(i)
#     img.save(i.replace('.tif', '.png'))
...
#     img.save(i.replace('_mask.tif', '.png'))
#     # img.save(i)
...
#         img_list = ["/".join(img_path.split('/')[-2:]) for img_path in img_list]  # noqa
#         img_list = [".".join(img_path.split('.')[:-1]) for img_path in img_list]  # noqa
...
#     if not os.path.exists(os.path.join(data_root, "masks/val")) and not os.path.exists(os.path.join(data_root, "masks/test")):  # noqa
#         all_imgs= sorted(glob.glob(data_root + "/images/train/*.png"))
#         x_train, x_val = train_test_split(all_imgs, test_size=0.2, random_state=0)  # noqa
...
#     import hashlib
#     all_imgs = []
#     for fpath, dirname, fnames in os.walk(os.path.join(data_root, "images")):  # noqa
#         for fname in fnames:
