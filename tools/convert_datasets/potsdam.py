...
def parse_args():
    ...
    parser.add_argument('--clip_size', type=int, help='potsdam clip size', default=512)
    parser.add_argument('--stride_size', type=int, help='potsdam overlap size', default=256)
...
def clip_big_image(image_path, clip_save_dir, to_label=False):
    ...
    num_rows = math.ceil((h - cs) / ss) if math.ceil((h - cs) / ss) * ss + cs >= h else math.ceil(
        (h - cs) / ss) + 1
    num_cols = math.ceil((w - cs) / ss) if math.ceil((w - cs) / ss) * ss + cs >= w else math.ceil(
        (w - cs) / ss) + 1
    ...
    boxes = np.stack([xmin + xmin_offset, ymin + ymin_offset,
                      np.minimum(xmin + cs, w), np.minimum(ymin + cs, h)], axis=1)

    if to_label:
        color_map = np.array([[0, 0, 0], [255, 255, 255], [0, 0, 255], [0, 255, 255], [0, 255, 0],
                              [255, 255, 0], [255, 0, 0]])
    ...
    for box in boxes:
        ...
        imsave(osp.join(clip_save_dir, '%s_%s_%d_%d_%d_%d.png' % (idx_i, idx_j, start_x, start_y, end_x, end_y)),
               clipped_image.astype(np.uint8), check_contrast=False)


def main():
    splits = {
        "train": ["2_10", "2_11", "2_12", "3_10", "3_11",
                  "3_12","4_10","4_11","4_12","5_10","5_11",
                  "5_12","6_10","6_11","6_12","6_7","6_8",
                  "6_9","7_10","7_11","7_12","7_7","7_8","7_9",
                  ],
        "val": [
            "5_15","6_15","6_13","3_13","4_14","6_14",
            "5_14","2_13","4_15","2_14","5_13","4_13",
            "3_14","7_13",
        ],
    }
    ...
    with tempfile.TemporaryDirectory(dir=args.tmp_dir) as tmp_dir:
        for zipp in zipp_list:
            ...
            for src_path in tqdm(src_path_list):
                ...
                data_type = 'train' if '%s_%s' % (idx_i, idx_j) in splits['train'] else 'val'
