...
def parse_args():
    ...
    parser.add_argument('--dataset_path', help='vaihingen folder path', default='D:\Vaihingen')
    parser.add_argument('--tmp_dir', help='path of the temporary directory')
    parser.add_argument('-o', '--out_dir', help='output path', default=r'D:\Vaihingen\data')
    parser.add_argument(
...
def clip_big_image(image_path, clip_save_dir, to_label=False):
    ...
    for box in boxes:
        ...
        imsave(
            osp.join(
                clip_save_dir, '%s_%d_%d_%d_%d.png' %
                (area_idx, start_x, start_y, end_x, end_y)),
    ...
    with tempfile.TemporaryDirectory(dir=args.tmp_dir) as tmp_dir:
        for zipp in zipp_list:
            ...
            if not len(src_path_list):
                sub_tmp_dir = os.path.join(tmp_dir, os.listdir(tmp_dir)[2])
