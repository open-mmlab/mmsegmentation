...
Usage:
    # Remap all PNGs in a directory (in-place):
    python tools/remap_pred_colors.py --src vis_pred/vis_data/vis_image/

    # Save remapped images to a different directory:
...
# ── color map ────────────────────────────────────────────────────────────────
# key   : (R, G, B) as produced by SegVisualizationHook with alpha=1.0
# value : (R, G, B) replacement
COLOR_MAP = {
...
def remap_image(img_array: np.ndarray) -> np.ndarray:
    """Apply COLOR_MAP to an H×W×3 uint8 array. Returns a new array."""
    out = img_array.copy()
...
def main():
    parser = argparse.ArgumentParser(
        description='Remap prediction image colors (black→#7c7c7c, red→#000bc5)')
    parser.add_argument(
        '--src', required=True,
        help='Source directory (or single PNG file) to process.')
    parser.add_argument(
        '--dst', default=None,
        help='Destination directory (or file). '
             'Defaults to in-place overwrite when omitted.')
    ...
    if src.is_file():
        ...
        if not files:
            ...
            sys.exit(0)
        if args.dst is None:
            pairs = [(f, f) for f in files]          # in-place
        else:
