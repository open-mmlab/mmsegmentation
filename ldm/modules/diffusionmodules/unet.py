...
from ldm.modules.attention import exists, Normalize, BasicTransformerBlock, MemoryEfficientCrossAttention, FeedForward, default
...
# 如果放在其他文件中，isinstance会有问题，可能是因为有相同名字的类
    ...
    def forward(self, x, emb, context=None):
        ...
        for layer in self:
            if isinstance(layer, TimestepBlock):
                ...
                x, cross_attn_maps = layer(x, context)
            else:
                x = layer(x)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        return self.to_out(out), cross_attn_map
...
class BasicTransformerBlock(nn.Module):
    ...
    def _forward(self, x, context=None):
        ...
        x2, cross_attn_map = self.attn2(self.norm2(x), context=context)
        x = x2 + x
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x, context=None, window_size=-1):
        ...
        for i, block in enumerate(self.transformer_blocks):
            if context[i] is not None:
                context[i] = context[i].unsqueeze(1).repeat(1, x.shape[0] // context[i].shape[0], 1, 1).flatten(0, 1)
            x, cross_attn_map = block(x, context=context[i])
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, y=None,**kwargs):
        ...
        for module in self.input_blocks:
            ...
            if h[1] is not None:
                all_cross_attn_maps.append(h[1])
            h = (h[0], None)
            hs.append(h[0])
        ...
        for module in self.output_blocks:
            h = (torch.cat([h[0], hs.pop()], dim=1), h[1])
            h = module(h, emb, context)
            if h[1] is not None:
                all_cross_attn_maps.append(h[1])
            h = (h[0], None)
        ...
        if self.predict_codebook_ids:
            ...
            return self.out(h), all_cross_attn_maps
    ...
    def forward(self, x, timesteps=None, context=None, control=None, only_mid_control=False, **kwargs):
        ...
        for i, module in enumerate(self.output_blocks):
            if only_mid_control or control is None:
                ...
                h = (torch.cat([h[0], hs.pop() + control.pop()], dim=1), h[1])
            h = module(h, emb, context)
            if h[1] is not None:
                all_cross_attn_maps.append(h[1])
            h = (h[0], None)
