...
def total_intersect_and_union(results: list,
    ...
    Args:
        results (list[torch.Tensor]): List of prediction segmentation maps.
        gt_seg_maps (list[torch.Tensor]): list of ground truth segmentation
        maps.
        num_classes (int): Number of categories.
        ignore_index (int): Index that will be ignored in evaluation.
...
def torch_mean_iou(results,
    ...
    """
    all_acc, acc, iou = torch_eval_metrics(
        results=results,
...
def torch_mean_dice(results,
    ...
    all_acc, acc, dice = torch_eval_metrics(
        results=results,
        gt_seg_maps=gt_seg_maps,
        num_classes=num_classes,
        ignore_index=ignore_index,
...
def torch_eval_metrics(results: list,
                       gt_seg_maps: list,
                       num_classes: int,
                       ignore_index: int,
                       metrics=['mIoU'],
    ...
    gt_seg_maps = [torch.from_numpy(gt_seg_map) for gt_seg_map in gt_seg_maps]
