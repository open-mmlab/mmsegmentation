...
def prepare_targets(targets, upper_ids, lower_ids):
    ...
    for targets_per_image in targets:
        mask = torch.zeros((h, w),
                           dtype=targets_per_image.dtype,
                           device=targets_per_image.device)
        indices_fore = torch.logical_and(targets_per_image >= fore_ids[0],
                                         targets_per_image <= fore_ids[-1])
        mask[indices_fore] = 1
        targets_top.append(mask)

    return targets, torch.stack(
        targets_middle, dim=0), torch.stack(
            targets_top, dim=0)
...
def losses_hiera(predictions,
    ...
    targets_middle = F.one_hot(
        targets_middle, num_classes=3).permute(0, 3, 1, 2)
    ...
    MCMB1 = torch.max(
        torch.cat(
            [predictions[:, ii:ii + 1, :, :] for ii in upper_ids] +
            [predictions[:, num_classes + 1:num_classes + 2, :, :]],
            dim=1), 1, True)[0]
    MCMB2 = torch.max(
        torch.cat(
            [predictions[:, ii:ii + 1, :, :] for ii in lower_ids] +
            [predictions[:, num_classes + 2:num_classes + 3, :, :]],
            dim=1), 1, True)[0]
    MCMB = torch.cat([MCMB_back, MCMB1, MCMB2], dim=1)
    MCMC_back = torch.max(
        torch.cat(
            [MCMB_back, predictions[:, num_classes + 3:num_classes + 4, :, :]],
            dim=1), 1, True)[0]
    MCMC1 = torch.max(
        torch.cat([
            MCMB1, MCMB2, predictions[:, num_classes + 4:num_classes + 5, :, :]
        ],
                  dim=1), 1, True)[0]
    ...
    MCLA_back = torch.min(
        torch.cat([predictions[:, 0:1, :, :], MCLB[:, 0:1, :, :]], dim=1), 1,
        True)[0]
    MCLA1 = torch.cat([
        torch.min(
            torch.cat([predictions[:, ii:ii + 1, :, :], MCLB[:, 1:2, :, :]],
                      dim=1), 1, True)[0] for ii in upper_ids
    ],
                      dim=1)
    MCLA2 = torch.cat([
        torch.min(
            torch.cat([predictions[:, ii:ii + 1, :, :], MCLB[:, 2:3, :, :]],
                      dim=1), 1, True)[0] for ii in lower_ids
    ],
                      dim=1)
    if len(upper_ids) > 5:
        MCLA = torch.cat([
            MCLA_back, MCLA1[:, 0:7, :, :], MCLA2[:, 0:2, :, :],
            MCLA1[:, 7:9, :, :], MCLA2[:, 2:3, :, :], MCLA1[:, 9:12, :, :],
            MCLA2[:, 3:7, :, :]
        ],
                         dim=1)
    else:
...
def losses_hiera_focal(predictions,
    ...
    loss = ((-targets[:, :num_classes, :, :] * torch.pow(
        (1.0 - MCLA), gamma) * torch.log(MCLA + eps) -
            ...
            valid_indices).sum() / num_valid / num_classes
    loss += ((-targets_middle * torch.pow(
        (1.0 - MCLB), gamma) * torch.log(MCLB + eps) -
              (1.0 - targets_middle) * torch.pow(MCMB, gamma) *
              torch.log(1 - MCMB + eps)) * valid_indices).sum() / num_valid / 3
    loss += ((-targets_top * torch.pow(
        (1 - MCLC), gamma) * torch.log(MCLC + eps) -
              (1.0 - targets_top) * torch.pow(MCMC, gamma) *
              torch.log(1 - MCMC + eps)) * valid_indices).sum() / num_valid / 2
...
class HieraLoss2(nn.Module):
    ...
    def forward(self, cls_score, label, weight=None, **kwargs):
        ...
        upper_ids = [1, 2, 3, 4]
        lower_ids = [5, 6]
        targets, targets_middle, targets_top = prepare_targets(
            label, upper_ids, lower_ids)
        loss = losses_hiera_focal(cls_score, targets, targets_middle,
                                  targets_top, self.num_classes, upper_ids,
                                  lower_ids)
