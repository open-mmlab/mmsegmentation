...
class DepthwiseSeparableObjectAttentionBlockv2(_SelfAttentionBlock):
    ...
    def __init__(self, in_channels, channels, scale, conv_cfg, norm_cfg,
        ...
        if scale > 1:
            ...
            query_downsample = None
        super(DepthwiseSeparableObjectAttentionBlockv2, self).__init__(
            key_in_channels=in_channels,
            query_in_channels=in_channels,
            channels=channels,
            out_channels=in_channels,
    ...
    def forward(self, query_feats, key_feats):
        """Forward function."""
        context = super(DepthwiseSeparableObjectAttentionBlockv2,
                        self).forward(query_feats, key_feats)
        output = self.bottleneck(torch.cat([context, query_feats], dim=1))
        if self.query_downsample is not None:
            output = resize(query_feats)
...
class DepthwiseSeparableObjectAttentionBlockv3(_DepthwiseSeparableSelfAttentionBlock):
    """Make a OCR used SelfAttentionBlock."""
    ...
    def __init__(self, in_channels, channels, scale, conv_cfg, norm_cfg,
        ...
        if scale > 1:
            ...
            query_downsample = None
        super(DepthwiseSeparableObjectAttentionBlockv3, self).__init__(
            key_in_channels=in_channels,
            query_in_channels=in_channels,
            channels=channels,
            out_channels=in_channels,
    ...
    def forward(self, query_feats, key_feats):
        """Forward function."""
        context = super(DepthwiseSeparableObjectAttentionBlockv3,
                        self).forward(query_feats, key_feats)
        output = self.bottleneck(torch.cat([context, query_feats], dim=1))
        if self.query_downsample is not None:
            output = resize(query_feats)
...
@HEADS.register_module()
class DepthwiseSeparableOCRHeadv4(BaseCascadeDecodeHead):
    """Object-Contextual Representations for Semantic Segmentation.
    ...
    """
    def __init__(self, ocr_channels, scale=1, **kwargs):
        super(DepthwiseSeparableOCRHeadv4, self).__init__(**kwargs)
        self.ocr_channels = ocr_channels
        self.scale = scale
        self.object_context_block = DepthwiseSeparableObjectAttentionBlockv3(
            self.channels,
            self.ocr_channels,
            self.scale,
            conv_cfg=self.conv_cfg,
