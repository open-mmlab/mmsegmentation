...
class SegLocalVisualizerConversion(Visualizer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.set_dataset_meta(
            classes=DatasetConverter.get_class_names(target_dataset),
            palette=DatasetConverter.get_palette(target_dataset),
    ...
    def _draw_sem_seg(self,
        ...
        unknown_idx = self.dataset_converter.unknown_idx
        ids = np.unique(sem_seg)[::-1]
        legal_indices = ids < num_classes 
        # legal_indices[ids == unknown_idx] = True
        ids_ = ids[legal_indices]
        labels = np.array(ids_, dtype=np.int64)
        ...
        if unknown_idx in ids:
            labels = np.append(labels, [unknown_idx])
    ...
    def convert_sem_seg_data(
        self,
        data_sample: SegDataSample
    ):
        converted_sample = SegDataSample(metainfo=data_sample.metainfo)
        
        if 'gt_sem_seg' in data_sample:
            gt_data = data_sample.gt_sem_seg.cpu().data
            gt_shape = gt_data.shape
            gt_converted = self.dataset_converter.convert_gt_label(
                label=np.asarray(gt_data.flatten())
            ) 
            gt_data = torch.tensor(gt_converted.reshape(gt_shape))
            gt_seg = PixelData(metainfo=data_sample.gt_sem_seg.metainfo)
            gt_seg.data = gt_data
            converted_sample.gt_sem_seg = gt_seg
            
            if 'pred_sem_seg' in data_sample:
                pred_data = data_sample.pred_sem_seg.cpu().data
                pred_shape = pred_data.shape
                pred_converted = self.dataset_converter.convert_pred_label(
                    pred_label=np.asarray(pred_data.flatten()),
                    gt_label_converted=gt_converted
                ) 
                pred_data = torch.tensor(pred_converted.reshape(pred_shape))
                pred_seg = PixelData(metainfo=data_sample.pred_sem_seg.metainfo)
                pred_seg.data = pred_data
                converted_sample.pred_sem_seg = pred_seg
    ...
    def add_datasample(
        ...
        """
        target_dataset = self.dataset_converter.target_dataset
        classes=DatasetConverter.get_class_names(target_dataset)
        palette=DatasetConverter.get_palette(target_dataset)
        # print(f"dataset_meta: {self.dataset_meta}")
        gt_img_data = None
        pred_img_data = None
        # print(f"\nvis: datasample pre converted:{data_sample}")
        data_sample = self.convert_sem_seg_data(data_sample=data_sample)
        # print(f"\nvis: datasample converted:\n{data_sample}")
